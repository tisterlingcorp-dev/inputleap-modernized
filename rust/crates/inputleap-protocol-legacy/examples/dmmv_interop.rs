...
fn run() -> Result<(), String> {
    ...
    match mode.to_str() {
        Some("decode-frame") => {
            ...
            if u32::from_be_bytes(bytes[..4].try_into().unwrap()) != 8 || &bytes[4..8] != b"DMMV" {
            ...
            if parsed.category != Category::Accepted
                ...
                || parsed.message != Some(RemoteMessage::MouseMove { x, y })
            {
                return Err(format!("unexpected Rust decode: {parsed:?}"));
            }
            println!("DMMV_RUST_DECODE_PASS x={x} y={y} frame_bytes=12");
        }
        Some("encode-frame") => {
            let mut payload = [0_u8; 8];
            if encode_remote_message_to(RemoteMessage::MouseMove { x, y }, context, &mut payload)
                != Ok(8)
            {
                return Err("Rust DMMV encode failed".into());
            }
            ...
            println!("DMMV_RUST_ENCODE_PASS x={x} y={y} frame_bytes=12");
        }
        _ => return Err(usage()),
    }
    Ok(())
...
fn usage() -> String {
    "usage: dmmv_interop <decode-frame|encode-frame> <i16-x> <i16-y> <path>".into()
}
fn main() {
    if let Err(error) = run() {
        eprintln!("dmmv_interop: {error}");
        std::process::exit(1);
    }
}
