...
fn run() -> Result<(), String> {
    ...
    match mode.to_str() {
        Some("decode-frame") => {
            ...
            if u32::from_be_bytes(bytes[..4].try_into().unwrap()) != 10 || &bytes[4..8] != b"DKDN" {
            ...
            println!(
                "DKDN_RUST_DECODE_PASS key_id={key_id} modifier_mask={modifier_mask} button={button} frame_bytes=14"
            );
        }
        Some("encode-frame") => {
            let mut payload = [0_u8; 10];
            if encode_remote_message_to(message, context, &mut payload) != Ok(10) {
                return Err("Rust DKDN encode failed".into());
            }
            ...
            println!(
                "DKDN_RUST_ENCODE_PASS key_id={key_id} modifier_mask={modifier_mask} button={button} frame_bytes=14"
            );
        }
        _ => return Err(usage()),
    }
...
fn usage() -> String {
    "usage: dkdn_interop <decode-frame|encode-frame> <u16-key-id> <u16-modifier-mask> <u16-button> <path>".into()
}

fn main() {
    if let Err(error) = run() {
        eprintln!("dkdn_interop: {error}");
        std::process::exit(1);
    }
}
