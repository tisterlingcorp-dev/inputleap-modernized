use inputleap_protocol_legacy::{
    Category, RemoteMessage, RemoteMessageContext, RemoteMessageDirection, RemoteMessageState,
    encode_remote_message_to, parse_remote_message,
};
use std::env;
use std::fs::{OpenOptions, metadata, read};
use std::io::Write;
use std::path::PathBuf;

fn run() -> Result<(), String> {
    let mut args = env::args_os().skip(1);
    let mode = args.next().ok_or_else(usage)?;
    let key_id = parse_u16(args.next(), "key-id")?;
    let modifier_mask = parse_u16(args.next(), "modifier-mask")?;
    let count = parse_u16(args.next(), "count")?;
    let button = parse_u16(args.next(), "button")?;
    let path = PathBuf::from(args.next().ok_or_else(usage)?);
    if args.next().is_some() {
        return Err(usage());
    }
    let context = RemoteMessageContext::new(
        RemoteMessageDirection::ServerToClient,
        RemoteMessageState::Active,
    );
    let message = RemoteMessage::KeyRepeat {
        key_id,
        modifier_mask,
        count,
        button,
    };
    match mode.to_str() {
        Some("decode-frame") => {
            if metadata(&path).map_err(|e| e.to_string())?.len() != 16 {
                return Err("frame must be exactly 16 bytes".into());
            }
            let bytes = read(&path).map_err(|e| e.to_string())?;
            if u32::from_be_bytes(bytes[..4].try_into().unwrap()) != 12 || &bytes[4..8] != b"DKRP" {
                return Err("invalid PacketStreamFilter frame".into());
            }
            let parsed = parse_remote_message(&bytes[4..], context);
            if parsed.category != Category::Accepted
                || parsed.consumed != Some(12)
                || parsed.message != Some(message)
            {
                return Err(format!("unexpected Rust decode: {parsed:?}"));
            }
            println!(
                "DKRP_RUST_DECODE_PASS key_id={key_id} modifier_mask={modifier_mask} count={count} button={button} frame_bytes=16"
            );
        }
        Some("encode-frame") => {
            let mut payload = [0_u8; 12];
            if encode_remote_message_to(message, context, &mut payload) != Ok(12) {
                return Err("Rust DKRP encode failed".into());
            }
            let mut frame = [0_u8; 16];
            frame[0..4].copy_from_slice(&12_u32.to_be_bytes());
            frame[4..].copy_from_slice(&payload);
            let mut output = OpenOptions::new()
                .write(true)
                .create_new(true)
                .open(&path)
                .map_err(|e| e.to_string())?;
            output.write_all(&frame).map_err(|e| e.to_string())?;
            output.flush().map_err(|e| e.to_string())?;
            println!(
                "DKRP_RUST_ENCODE_PASS key_id={key_id} modifier_mask={modifier_mask} count={count} button={button} frame_bytes=16"
            );
        }
        _ => return Err(usage()),
    }
    Ok(())
}

fn parse_u16(value: Option<std::ffi::OsString>, name: &str) -> Result<u16, String> {
    value
        .ok_or_else(usage)?
        .to_str()
        .ok_or_else(usage)?
        .parse()
        .map_err(|_| format!("{name} must be u16"))
}

fn usage() -> String {
    "usage: dkrp_interop <decode-frame|encode-frame> <u16-key-id> <u16-modifier-mask> <u16-count> <u16-button> <path>".into()
}

fn main() {
    if let Err(error) = run() {
        eprintln!("dkrp_interop: {error}");
        std::process::exit(1);
    }
}
