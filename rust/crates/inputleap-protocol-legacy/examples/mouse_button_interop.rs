...
const MOUSE_BUTTON_PAYLOAD_LENGTH: usize = 5;
const MOUSE_BUTTON_FRAME_LENGTH: usize = 9;
...
fn require_mouse_button(
    payload: &[u8],
    message_context: RemoteMessageContext,
) -> Result<(&'static str, u8), String> {
    let parsed = parse_remote_message(payload, message_context);
    let Some(message) = parsed.message else {
        return Err(format!(
            "payload was not accepted as DMDN or DMUP in the requested context: {parsed:?}"
        ));
    };
    let (kind, button) = match message {
        RemoteMessage::MouseDown { button } => ("down", button),
        RemoteMessage::MouseUp { button } => ("up", button),
        _ => {
            return Err(format!(
                "payload was not accepted as DMDN or DMUP in the requested context: {parsed:?}"
            ));
    ...
    if parsed.category != Category::Accepted || parsed.consumed != Some(MOUSE_BUTTON_PAYLOAD_LENGTH)
    {
        return Err(format!("unexpected DMDN/DMUP parse boundary: {parsed:?}"));
    }
    if message.is_terminal() {
        return Err("DMDN/DMUP must be nonterminal parser metadata".to_owned());
    }
    Ok((kind, button))
}
...
fn decode(args: &[OsString]) -> Result<(), String> {
    if args.len() != 5 {
        return Err(
            "usage: mouse_button_interop decode <path> <direction> <receiver-state>".to_owned(),
        );
    }
    let path = PathBuf::from(&args[2]);
    let message_context = context(&args[3], &args[4])?;
    let bytes = read_exact_file::<MOUSE_BUTTON_PAYLOAD_LENGTH>(&path)?;
    let (kind, button) = require_mouse_button(&bytes, message_context)?;
    println!(
        "RUST_MOUSE_BUTTON_DECODE_PASS consumed={MOUSE_BUTTON_PAYLOAD_LENGTH} kind={kind} button={button} terminal=false stateful_effects=NOT_EXECUTED"
    );
    Ok(())
}
...
fn decode_frame(args: &[OsString]) -> Result<(), String> {
    if args.len() != 5 {
        return Err(
            "usage: mouse_button_interop decode-frame <path> <direction> <receiver-state>"
                .to_owned(),
        );
    }
    let path = PathBuf::from(&args[2]);
    let message_context = context(&args[3], &args[4])?;
    let bytes = read_exact_file::<MOUSE_BUTTON_FRAME_LENGTH>(&path)?;
    ...
    if consumed != MOUSE_BUTTON_FRAME_LENGTH {
        return Err(format!("unexpected outer-frame boundary: {consumed}"));
    }
    let (kind, button) = require_mouse_button(payload, message_context)?;
    println!(
        "RUST_MOUSE_BUTTON_FRAME_DECODE_PASS frame_consumed={consumed} payload_consumed={MOUSE_BUTTON_PAYLOAD_LENGTH} kind={kind} button={button} terminal=false stateful_effects=NOT_EXECUTED"
    );
    Ok(())
}
...
fn encode(args: &[OsString]) -> Result<(), String> {
    if args.len() != 7 {
        return Err(
            "usage: mouse_button_interop encode <down|up> <new-path> <direction> <receiver-state> <button-u8>"
                .to_owned(),
        );
    }
    let (kind, message) = match args[2].to_str() {
        Some("down") => (
            "down",
            RemoteMessage::MouseDown {
                button: parse_u8(&args[6], "button-u8")?,
            },
        ),
        Some("up") => (
            "up",
        ...
        _ => return Err("kind must be down or up".to_owned()),
    };
    let button = match message {
        RemoteMessage::MouseDown { button } | RemoteMessage::MouseUp { button } => button,
        _ => unreachable!(),
    };
    let path = PathBuf::from(&args[3]);
    let message_context = context(&args[4], &args[5])?;
    let mut bytes = [0_u8; MOUSE_BUTTON_PAYLOAD_LENGTH];
    let written = encode_remote_message_to(message, message_context, &mut bytes)
        .map_err(|error| format!("encode rejected requested message: {error:?}"))?;
    if written != MOUSE_BUTTON_PAYLOAD_LENGTH {
        return Err(format!("unexpected DMDN/DMUP encoded length: {written}"));
    }
    ...
    println!(
        "RUST_MOUSE_BUTTON_ENCODE_PASS bytes={written} kind={kind} button={button} terminal=false stateful_effects=NOT_EXECUTED path={}",
        path.display()
    );
    Ok(())
}
...
fn run() -> Result<(), String> {
    ...
    match args.get(1).and_then(|value| value.to_str()) {
        ...
        _ => Err("usage: mouse_button_interop <decode|decode-frame|encode> ...".to_owned()),
    }
}

fn main() {
    if let Err(error) = run() {
        eprintln!("mouse_button_interop: {error}");
        std::process::exit(1);
    }
}
