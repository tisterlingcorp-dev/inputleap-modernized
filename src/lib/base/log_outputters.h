...
class FileLogOutputterState {
public:
    FileLogOutputterState() = default;
    explicit FileLogOutputterState(std::string filename) : filename_(std::move(filename)) {}
    ...
    decltype(auto) withFilename(Operation&& operation)
    {
        std::lock_guard<std::mutex> lock(mutex_);
        return std::forward<Operation>(operation)(filename_);
    }
