...
void
ConsoleLogOutputter::open(const char*)
{
}
...
FileLogOutputter::write(ELevel level, const char *message)
{
    ...
    return m_state.withFilename([&](const std::string& fileName) {
        bool moveFile = false;

        std::ofstream handle;
        inputleap::open_utf8_path(handle, fileName, std::fstream::app);
        if (handle.is_open() && handle.fail() != true) {
            handle << message << std::endl;

            // when file size exceeds limits, move to 'old log' filename.
            size_t p = handle.tellp();
        ...
        if (moveFile) {
            std::string oldLogFilename = inputleap::string::sprintf("%s.1", fileName.c_str());
            remove(oldLogFilename.c_str());
            rename(fileName.c_str(), oldLogFilename.c_str());
        }
