...
void ClientProxy1_6::setClipboard(ClipboardID id, const IClipboard* clipboard)
{
    ...
    if (m_clipboard[id].m_dirty) {
        ...
        if (m_clipboard[id].m_clipboard.open(0)) {
            if (m_clipboard[id].m_clipboard.has(IClipboard::kFileList)) {
                LOG_INFO("setClipboard file list bytes=%zu package=%s",
                    m_clipboard[id].m_clipboard.get(IClipboard::kFileList).size(),
                    DragInformation::isFilePackage(m_clipboard[id].m_clipboard.get(IClipboard::kFileList)) ? "yes" : "no");
            }
