	...
	if (m_active != dst) {
		...
		if (m_enableClipboard) {
			...
			for (ClipboardID id = 0; id < kClipboardEnd; ++id) {
				...
				Clipboard outgoingClipboard;
				outgoingClipboard.unmarshall(m_clipboards[id].m_clipboardData, 0);
				m_active->setClipboard(id, &outgoingClipboard);
			}
		}
...
void Server::onClipboardChanged(BaseClientProxy* sender, ClipboardID id, std::uint32_t seqNum)
{
    ...
    // A primary-screen file list is converted to a content package before it
    // enters the network state. Remote clients already send this package.
    Clipboard outgoing;
    const IClipboard* outgoingClipboard = &clipboard.m_clipboard;
    if (sender == m_primaryClient && clipboard.m_clipboard.open(0) &&
        clipboard.m_clipboard.has(IClipboard::kFileList)) {
        const std::string fileList = clipboard.m_clipboard.get(IClipboard::kFileList);
        clipboard.m_clipboard.close();
        std::string package;
        LOG_INFO("packaging clipboard file list: %s", fileList.c_str());
        const bool packaged = DragInformation::packageFileList(fileList, package);
        LOG_INFO("packaging clipboard file list result: %s, bytes=%zu",
                 packaged ? "yes" : "no", package.size());
        if (!packaged ||
            !outgoing.open(0) || !outgoing.clear()) {
            LOG_WARN("rejecting clipboard file list: failed to package local files");
            return;
        }
        if (clipboard.m_clipboard.open(0)) {
            for (std::int32_t format = 0; format != IClipboard::kNumFormats; ++format) {
                const auto current = static_cast<IClipboard::EFormat>(format);
                if (current != IClipboard::kFileList && clipboard.m_clipboard.has(current)) {
                    outgoing.add(current, clipboard.m_clipboard.get(current));
                }
            }
            clipboard.m_clipboard.close();
        }
        outgoing.add(IClipboard::kFileList, package);
        outgoing.close();
        outgoingClipboard = &outgoing;
    }

    std::string outgoingData = IClipboard::marshall(outgoingClipboard);
    if (outgoingData.size() > m_maximumClipboardSize) {
        LOG_NOTE("not updating clipboard because it's over the size limit (%zi KB) configured by the server",
            m_maximumClipboardSize);
        return;
    }
    if (outgoingData == clipboard.m_clipboardData) {
        LOG_DEBUG("ignored screen \"%s\" update of clipboard %d (unchanged)", clipboard.m_clipboardOwner.c_str(), id);
        return;
    }

    clipboard.m_clipboardData = outgoingData;
	...
	// A package is extracted only at its destination.  This prevents source
    // paths from crossing machines and keeps the local clipboard usable.
    bool activeHasPackage = false;
    if (outgoingClipboard->open(0)) {
        activeHasPackage = outgoingClipboard->has(IClipboard::kFileList) &&
            DragInformation::isFilePackage(outgoingClipboard->get(IClipboard::kFileList));
        outgoingClipboard->close();
    }
    if (m_active == m_primaryClient && sender == m_primaryClient) {
        // The source already owns the native clipboard; do not replace it
        // with the transport package.
    } else if (outgoingClipboard == &outgoing) {
        LOG_INFO("sending packaged clipboard to active client");
        m_active->setClipboard(id, &outgoing);
    } else if (m_active == m_primaryClient && activeHasPackage) {
        Clipboard localClipboard;
        std::string localFiles;
        if (!clipboard.m_clipboard.open(0) || !clipboard.m_clipboard.has(IClipboard::kFileList)) {
            LOG_WARN("rejecting clipboard file package: unavailable payload");
            return;
        }
        const std::string package = clipboard.m_clipboard.get(IClipboard::kFileList);
        clipboard.m_clipboard.close();
        if (!DragInformation::unpackFilePackage(package, m_screen->getDropTarget(), localFiles) ||
            !localClipboard.open(0) || !localClipboard.clear()) {
            LOG_WARN("rejecting clipboard file package: failed safe extraction");
            return;
        }
        if (clipboard.m_clipboard.open(0)) {
            for (std::int32_t format = 0; format != IClipboard::kNumFormats; ++format) {
                const auto current = static_cast<IClipboard::EFormat>(format);
                if (current != IClipboard::kFileList && clipboard.m_clipboard.has(current)) {
                    localClipboard.add(current, clipboard.m_clipboard.get(current));
                }
            }
            clipboard.m_clipboard.close();
        }
        localClipboard.add(IClipboard::kFileList, localFiles);
        localClipboard.close();
        m_active->setClipboard(id, &localClipboard);
    } else {
        m_active->setClipboard(id, outgoingClipboard);
    }
}
...
Server::closeClient(BaseClientProxy* client, const char* msg)
{
	...
	const std::string disconnectedScreen = getName(client);
	const bool removed = removeClient(client);
	emitActiveClientRemovalEvents(disconnectedScreen, removed, false);
...
Server::removeActiveClient(BaseClientProxy* client)
{
	const std::string disconnectedScreen = getName(client);
	const bool removed = removeClient(client);
	emitActiveClientRemovalEvents(disconnectedScreen, removed, true);
	if (removed) {
		forceLeaveClient(client);
        m_events->remove_handler(EventType::CLIENT_PROXY_DISCONNECTED, client);
	}
...
void
Server::emitActiveClientRemovalEvents(const std::string& screenName, bool removed,
                                      bool allowServerDisconnected)
{
	const auto decision = ServerConnectionEventPolicy::activeClientRemoved(
		screenName, removed, allowServerDisconnected, m_clients.size(), m_oldClients.size());
	if (decision.screenDisconnected) {
		notifyScreenDisconnected(*decision.screenDisconnected);
	}
	if (decision.serverDisconnected) {
        m_events->add_event(EventType::SERVER_DISCONNECTED, this);
	}
}

void
Server::notifyScreenDisconnected(const std::string& screenName)
{
	Server::ScreenConnectedInfo info{screenName};
    m_events->add_event(EventType::SERVER_SCREEN_DISCONNECTED, this,
                        create_event_data<Server::ScreenConnectedInfo>(info));
}
