...
void ClientListener::handle_client_connecting()
{
    ...
    m_events->add_handler(EventType::CLIENT_LISTENER_ACCEPT_FAILED,
                          socket_ptr->get_event_target(),
                          [this, socket_ptr](const auto& e)
    {
        handle_client_accept_failed(socket_ptr);
    });
...
void ClientListener::handle_client_accepted(IDataSocket* socket_ptr)
{
    ...
    m_events->remove_handler(EventType::CLIENT_LISTENER_ACCEPT_FAILED,
                             socket_ptr->get_event_target());
...
void ClientListener::handle_client_accept_failed(IDataSocket* socket_ptr)
{
    m_events->remove_handler(EventType::CLIENT_LISTENER_ACCEPTED,
                             socket_ptr->get_event_target());
    m_events->remove_handler(EventType::CLIENT_LISTENER_ACCEPT_FAILED,
                             socket_ptr->get_event_target());
    client_sockets_.erase(socket_ptr);
}
