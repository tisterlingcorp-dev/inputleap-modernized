...
{
    if (key == nullptr || name == nullptr) {
        throw std::invalid_argument("registry key and value name are required");
    }
    if (value.size() >= static_cast<std::size_t>(MAXDWORD)) {
        throw std::length_error("registry string value is too large");
    }

    const DWORD valueSize = static_cast<DWORD>(value.size() + 1);
    LONG result = RegSetValueEx(
        key, name, 0, REG_SZ,
        reinterpret_cast<const BYTE*>(value.c_str()), valueSize);
    if (result != ERROR_SUCCESS) {
        throw std::system_error(result, std::system_category(), "failed to write registry value");
    }

    DWORD persistedType = 0;
    DWORD persistedSize = 0;
    result = RegQueryValueEx(key, name, nullptr, &persistedType, nullptr, &persistedSize);
    if (result != ERROR_SUCCESS) {
        throw std::system_error(result, std::system_category(), "failed to read back registry value");
    }
    std::vector<BYTE> persisted(persistedSize);
    result = RegQueryValueEx(
        key, name, nullptr, &persistedType, persisted.data(), &persistedSize);
    if (result != ERROR_SUCCESS) {
        throw std::system_error(result, std::system_category(), "failed to read back registry value");
    }
    if (persistedType != REG_SZ || persistedSize != valueSize ||
        std::memcmp(persisted.data(), value.c_str(), valueSize) != 0) {
        throw std::runtime_error("registry value readback mismatch");
    }

    result = RegFlushKey(key);
    if (result != ERROR_SUCCESS) {
        throw std::system_error(result, std::system_category(), "failed to flush registry value");
    }
}
