...
public:
    ...
    // Clipboard file transfer uses a bounded, length-prefixed package over
    // the existing clipboard chunk stream.  File names are stored as base
    // names only; source paths never cross the trust boundary.
    static bool packageFileList(const std::string& fileList, std::string& package);
    static bool isFilePackage(const std::string& data);
    static bool unpackFilePackage(const std::string& package,
