...
bool
App::sendIpcConnectionState(IpcConnectionState state, IpcConnectionRole role,
                                 IpcIdentityPresence identityPresence,
                                 const std::string& technicalName,
                                 const std::string& detail)
{
    if (!argsBase().m_enableIpc || m_ipcClient == nullptr) {
        return false;
    }
    const auto result = m_ipcClient->sendConnectionState(IpcConnectionStateMessage(
        state, role, identityPresence, technicalName, detail));
    return result != IpcClient::SendResult::Rejected;
}
