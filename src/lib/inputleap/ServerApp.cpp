...
ServerApp::help()
{
    ...
    auto usr_config_path = inputleap::path_to_utf8(profile_path / inputleap::path_from_utf8(CONFIG_NAME));
    auto sys_config_path = inputleap::path_to_utf8(inputleap::DataDirectories::systemconfig() /
                                                   inputleap::path_from_utf8(CONFIG_NAME));
...
ServerApp::loadConfig()
{
    ...
    // load the default configuration if no explicit file given
        ...
        for (const auto& path_config : path_configs) {
            ...
            if (!path.empty()) {
                path /= inputleap::path_from_utf8(path_config.filename);
                if (loadConfig(inputleap::path_to_utf8(path))) {
                    if (path_config.deprecated) {
                        LOG_PRINT("%s: Loading config from deprecated path %s, please use %s",
                                  args().m_exename.c_str(),
                                  inputleap::path_to_utf8(path).c_str(),
                                  inputleap::path_to_utf8(path_configs[0].root / path_configs[0].filename).c_str());
                    }
                    loaded = true;
                    args().m_configFile = inputleap::path_to_utf8(path);
                    break;
                }
            }
        }
...
std::unique_ptr<Server> ServerApp::open_server(Config& config, PrimaryClient* primaryClient)
{
    ...
    m_events->add_handler(EventType::SERVER_CONNECTED, m_primaryClient->get_event_target(),
                          [this](const auto& e) {
        const auto& info = e.template get_data_as<Server::ScreenConnectedInfo>();
        sendIpcConnectionState(IpcConnectionState::Connected, IpcConnectionRole::ServerPeer,
                               IpcIdentityPresence::Known, info.m_screen);
    });
    m_events->add_handler(EventType::SERVER_SCREEN_DISCONNECTED, server.get(),
                          [this](const auto& e) {
        const auto& info = e.template get_data_as<Server::ScreenConnectedInfo>();
        sendIpcConnectionState(IpcConnectionState::Disconnected, IpcConnectionRole::ServerPeer,
                               IpcIdentityPresence::Known, info.m_screen);
    });
