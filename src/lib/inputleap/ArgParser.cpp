...
Argv::Argv(int argc, const char* const* argv) :
    ...
    m_exename(inputleap::path_to_utf8(inputleap::path_from_utf8(argv[0]).filename()))
{
...
ArgParser::parseGenericArgs(Argv& argv)
{
    ...
    if (argv.shift("-d", "--debug", &optarg)) {
        ...
        argsBase().m_profileDirectory = inputleap::path_from_utf8(optarg);
    }
    else if (argv.shift("--plugin-dir", nullptr, &optarg)) {
        argsBase().m_pluginDirectory = inputleap::path_from_utf8(optarg);
    }
    else {
        // option not supported here
        return false;
...
{
    // FIXME: we assume UTF-8 encoding, but on Windows this is not correct
    return inputleap::path_to_utf8(inputleap::path_from_utf8(arg).filename());
}
