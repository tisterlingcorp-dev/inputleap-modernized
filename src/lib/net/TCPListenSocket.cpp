...
int TCPListenSocket::boundPort() const
{
    sockaddr_storage address{};
#ifdef SYSAPI_WIN32
    int length = sizeof(address);
    if (::getsockname(m_socket->m_socket, reinterpret_cast<sockaddr*>(&address),
                      &length) != 0) {
        throw XSocketBind("could not query bound socket address");
    }
#else
    socklen_t length = sizeof(address);
    if (::getsockname(m_socket->m_fd, reinterpret_cast<sockaddr*>(&address),
                      &length) != 0) {
        throw XSocketBind("could not query bound socket address");
    }
#endif
    ...
    throw XSocketBind("bound socket has an unsupported address family");
}

std::unique_ptr<IDataSocket> TCPListenSocket::accept()
{
    std::unique_ptr<IDataSocket> socket;
    ArchNetAddress remoteAddress = nullptr;
    try {
        socket = std::make_unique<TCPSocket>(m_events, m_socketMultiplexer,
                                             ARCH->acceptSocket(m_socket, &remoteAddress));
        if (remoteAddress != nullptr) {
            LOG_NOTE("accepted client connection from %s", ARCH->addrToString(remoteAddress).c_str());
            ARCH->closeAddr(remoteAddress);
        }
        setListeningJob();
        return socket;
    }
