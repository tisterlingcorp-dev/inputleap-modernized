...
SecureSocket::secureWrite(const void* buffer, int size, int& wrote)
{
    ...
    if (m_ssl->m_ssl != nullptr) {
        LOG_DEBUG2("writing secure socket:%p", static_cast<void*>(this));
...
{
    std::lock_guard<std::mutex> ssl_lock{ssl_mutex_};
    const std::string path_utf8 = inputleap::path_to_utf8(path);
    ...
    if (path.empty()) {
        ...
        if (!inputleap::fs::is_regular_file(path)) {
            showError("ssl certificate doesn't exist: " + path_utf8);
            return false;
        }
    }

    int r = 0;
    r = SSL_CTX_use_certificate_file(m_ssl->m_context, path_utf8.c_str(), SSL_FILETYPE_PEM);
    if (r <= 0) {
        showError("could not use ssl certificate: " + path_utf8);
        return false;
    }

    r = SSL_CTX_use_PrivateKey_file(m_ssl->m_context, path_utf8.c_str(), SSL_FILETYPE_PEM);
    if (r <= 0) {
        showError("could not use ssl private key: " + path_utf8);
        return false;
    }

    r = SSL_CTX_check_private_key(m_ssl->m_context);
    if (!r) {
        showError("could not verify ssl private key: " + path_utf8);
        return false;
    }
...
bool SecureSocket::verify_peer_certificate(const inputleap::fs::path& fingerprint_db_path)
{
    ...
    const std::string fingerprint_db_path_utf8 = inputleap::path_to_utf8(fingerprint_db_path);
    LOG_NOTE("fingerprint_db_path: %s", fingerprint_db_path_utf8.c_str());
    ...
    if (!db.fingerprints().empty()) {
        LOG_NOTE("Read %zd fingerprints from: %s", db.fingerprints().size(),
             fingerprint_db_path_utf8.c_str());
    } else {
                                                 ...
                                                 bool read, bool write, bool error)
{
    ...
    // If status < 0, error happened
    if (status < 0) {
        sendEvent(EventType::CLIENT_LISTENER_ACCEPT_FAILED);
        return {false, {}};
    }
