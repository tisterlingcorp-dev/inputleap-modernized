...
public:
    ...
    */
    ...
    //! Construct a listener address; port zero requests an OS-assigned port.
    static NetworkAddress forListener(const std::string& hostname, int port);
...
private:
    NetworkAddress(const std::string& hostname, int port, bool allowZeroPort);
    void checkPort(bool allowZeroPort = false);
