    ...
    m_socketMultiplexer(socketMultiplexer)
{
    ...
    LOG_DEBUG("Opening new socket: %p", static_cast<void*>(m_socket));
...
TCPSocket::close()
{
    LOG_DEBUG("Closing socket: %p", static_cast<void*>(m_socket));

    // Reject late job registrations before removing the current job. This
    // keeps a concurrent writer from reinstalling a job after removal.
    job_registration_.close([this] { removeJob(); });
...
std::uintptr_t TCPSocket::nativeHandle() const
{
#ifdef SYSAPI_WIN32
    return m_socket == nullptr ? 0 : static_cast<std::uintptr_t>(m_socket->m_socket);
#else
    return 0;
#endif
}
...
void TCPSocket::setJob(std::unique_ptr<ISocketMultiplexerJob>&& job)
{
    if (job.get() == nullptr) {
        ...
        job_registration_.registerJob([this, &job] {
