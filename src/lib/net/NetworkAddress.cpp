    ...
    m_port(port)
{
    ...
    checkPort(allowZeroPort);
}

NetworkAddress NetworkAddress::forListener(const std::string& hostname, int port)
{
    return NetworkAddress(hostname, port, true);
}
...
void
NetworkAddress::checkPort(bool allowZeroPort)
{
    // check port number
    if (m_port < (allowZeroPort ? 0 : 1) || m_port > 65535) {
