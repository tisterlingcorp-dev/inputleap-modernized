...
ServerProxy::resetKeepAliveAlarm()
{
    ...
    if (m_keepAliveAlarm > 0.0) {
        ...
        m_events->add_handler(EventType::TIMER, m_keepAliveAlarmTimer,
                              [this](const auto&){ handle_keep_alive_alarm(); });
    }
}
...
ServerProxy::keyDown()
{
    ...
    if (!ProtocolUtil::readf(m_stream, kMsgDKeyDown + 4, &id, &mask, &button)) {
        throw XBadClient("truncated key down message");
    }
...
ServerProxy::keyRepeat()
{
    ...
    if (!ProtocolUtil::readf(m_stream, kMsgDKeyRepeat + 4,
                             &id, &mask, &count, &button)) {
        throw XBadClient("truncated key repeat message");
    }
...
ServerProxy::keyUp()
{
    ...
    // parse
    std::uint16_t id, mask, button;
    if (!ProtocolUtil::readf(m_stream, kMsgDKeyUp + 4, &id, &mask, &button)) {
        throw XBadClient("truncated key up message");
    }
...
ServerProxy::mouseDown()
{
    ...
    // parse
    std::int8_t id;
    if (!ProtocolUtil::readf(m_stream, kMsgDMouseDown + 4, &id)) {
        throw XBadClient("truncated mouse down message");
    }
...
ServerProxy::mouseUp()
{
    ...
    // parse
    std::int8_t id;
    if (!ProtocolUtil::readf(m_stream, kMsgDMouseUp + 4, &id)) {
        throw XBadClient("truncated mouse up message");
    }
...
ServerProxy::mouseMove()
{
    ...
    if (!ProtocolUtil::readf(m_stream, kMsgDMouseMove + 4, &rawX, &rawY)) {
        throw XBadClient("truncated mouse move message");
    }
    const auto x = static_cast<std::int16_t>(rawX);
    const auto y = static_cast<std::int16_t>(rawY);
...
ServerProxy::mouseRelativeMove()
{
    // parse
    bool ignore;
    std::uint16_t rawDx, rawDy;
    if (!ProtocolUtil::readf(m_stream, kMsgDMouseRelMove + 4, &rawDx, &rawDy)) {
        throw XBadClient("truncated relative mouse move message");
    }
    const auto dx = static_cast<std::int16_t>(rawDx);
    const auto dy = static_cast<std::int16_t>(rawDy);
