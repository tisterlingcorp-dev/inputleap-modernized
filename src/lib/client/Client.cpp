...
{
    if (clipboard != nullptr && clipboard->open(0)) {
        const bool hasFiles = clipboard->has(IClipboard::kFileList);
        const std::string fileData = hasFiles ? clipboard->get(IClipboard::kFileList) : std::string();
        clipboard->close();
        LOG_INFO("received clipboard: has file list=%s, bytes=%zu, drop target=%s",
                 hasFiles ? "yes" : "no", fileData.size(),
                 m_screen->getDropTarget().c_str());
        const bool isFilePackage = hasFiles && DragInformation::isFilePackage(fileData);
        LOG_INFO("received clipboard file list package=%s, prefix=%s",
                 isFilePackage ? "yes" : "no",
                 fileData.substr(0, std::min<std::size_t>(8, fileData.size())).c_str());
        if (isFilePackage) {
            if (id == kClipboardSelection) {
                // File copy/paste belongs to the X11 CLIPBOARD selection.
                // Leave PRIMARY untouched: clearing or replacing it generates
                // a local ownership event that can race with and invalidate
                // the file clipboard while Nautilus is resolving its URIs.
                LOG_DEBUG("ignoring file list for X11 PRIMARY selection");
            ...
            if (home == nullptr || *home == '\0') {
                LOG_WARN("rejecting clipboard file package: HOME is unavailable");
                return;
            }
            // Keep the received files out of the visible desktop.  The X11
            // clipboard advertises these paths and the file manager copies
            // them to the actual destination only when the user presses
            // Ctrl+V.
            const std::string clipboardCache =
                std::string(home) + "/.cache/inputleap/clipboard";
            std::string localFiles;
            if (!DragInformation::unpackFilePackage(fileData, clipboardCache, localFiles)) {
                LOG_WARN("rejecting clipboard file package: failed safe extraction");
                return;
            }
            LOG_INFO("staged clipboard file list in cache: %s", localFiles.c_str());
            Clipboard localClipboard;
            if (!localClipboard.open(0) || !localClipboard.clear()) {
                LOG_WARN("rejecting clipboard file package: local clipboard unavailable");
                return;
            }
            if (clipboard->open(0)) {
                for (std::int32_t format = 0; format != IClipboard::kNumFormats; ++format) {
                    const auto current = static_cast<IClipboard::EFormat>(format);
                    if (current != IClipboard::kFileList && clipboard->has(current)) {
                        localClipboard.add(current, clipboard->get(current));
                    }
                }
                clipboard->close();
            }
            ...
            m_screen->setClipboard(id, &localClipboard);
        } else {
...
Client::sendClipboard(ClipboardID id)
{
    ...
    if (m_timeClipboard[id] == 0 ||
        ...
        // Package copied files as content, never as source paths.  The
        // package travels through the existing chunked secure clipboard
        // stream and is materialized by the receiving screen.
        Clipboard outgoing;
        const IClipboard* outgoingClipboard = &clipboard;
        if (clipboard.open(0) && clipboard.has(IClipboard::kFileList)) {
            const std::string fileList = clipboard.get(IClipboard::kFileList);
            clipboard.close();
            std::string package;
            if (!DragInformation::packageFileList(fileList, package)) {
                LOG_WARN("rejecting clipboard file list: failed to package local files");
                return;
            }
            if (!outgoing.open(0) || !outgoing.clear()) {
                LOG_WARN("rejecting clipboard file list: transport clipboard unavailable");
                return;
            }
            if (clipboard.open(0)) {
                for (std::int32_t format = 0; format != IClipboard::kNumFormats; ++format) {
                    const auto current = static_cast<IClipboard::EFormat>(format);
                    if (current != IClipboard::kFileList && clipboard.has(current)) {
                        outgoing.add(current, clipboard.get(current));
                    }
                }
                clipboard.close();
            }
        ...
        std::string data = IClipboard::marshall(outgoingClipboard);
        ...
        if (!m_sentClipboard[id] || data != m_dataClipboard[id]) {
            ...
            m_server->onClipboardChanged(id, outgoingClipboard);
        }
    }
}
...
Client::setupConnection()
{
    ...
    m_events->add_handler(EventType::SOCKET_STOP_RETRY, m_stream->get_event_target(),
                          [this](const auto&){ handle_stop_retry(); });
}
...
Client::setupScreen()
{
    ...
    m_events->add_handler(EventType::SCREEN_SHAPE_CHANGED, get_event_target(),
                          [this](const auto&){ handle_shape_changed(); });
...
Client::setupTimer()
{
    ...
    m_events->add_handler(EventType::TIMER, m_timer,
                          [this](const auto&){ handle_connect_timeout(); });
}
...
#if WINAPI_XWINDOWS
    // X11 has no platform implementation for synthesizing an OLE drag.
    // The received file chunks are written directly to the configured drop
    // directory when the transfer completes.
    LOG_INFO("received drag information; files will be written to the drop directory");
#else
