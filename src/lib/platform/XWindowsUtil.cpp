                                     ...
                                     bool deleteProperty)
{
    ...
    while (bytesLeft != 0) {
        ...
        default:
            numBytes = numItems;
            // XGetWindowProperty() expresses the offset in 32-bit units,
            // even when the property format is 8.  Round up so a short
            // final chunk cannot be requested again on the next iteration.
            offset  += (numItems + 3) / 4;
            break;
