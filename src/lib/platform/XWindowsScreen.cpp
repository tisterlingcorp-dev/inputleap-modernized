...
XWindowsScreen::getDropTarget() const
{
    ...
    if (home == nullptr || *home == '\0') {
        LOG_ERR("failed to get HOME, no drop target available");
        return m_dropTargetPath;
    }

    const std::string configPath = std::string(home) + "/.config/user-dirs.dirs";
    std::ifstream config(configPath);
    std::string line;
    while (std::getline(config, line)) {
        const std::string prefix = "XDG_DESKTOP_DIR=\"";
        ...
        m_dropTargetPath = line.substr(valueStart, valueEnd - valueStart);
        const std::string homeToken = "$HOME";
        if (m_dropTargetPath.compare(0, homeToken.size(), homeToken) == 0) {
            m_dropTargetPath.replace(0, homeToken.size(), home);
        }
        break;
    }

    if (m_dropTargetPath.empty()) {
        m_dropTargetPath = std::string(home) + "/Desktop";
    }
...
void
XWindowsScreen::setDropTarget(const std::string& target)
{
