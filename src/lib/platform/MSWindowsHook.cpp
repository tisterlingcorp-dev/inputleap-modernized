...
static MSWindowsHookMode g_mode{kHOOK_DISABLE};
...
{
    const auto mode = g_mode.snapshot();
    ...
    if (mode.value() != kHOOK_RELAY_EVENTS) {
...
keyboardLLHook(int code, WPARAM wParam, LPARAM lParam)
{
    ...
    if (code == HC_ACTION && !is_immune_key(info->vkCode)) {
        WPARAM keyParam = info->vkCode;
        LPARAM keyData = 1;                          // repeat code
        keyData |= (info->scanCode << 16);           // scan code
        if (info->flags & LLKHF_EXTENDED) {
            keyData |= (1lu << 24);                  // extended key
        }
        if (info->flags & LLKHF_ALTDOWN) {
            keyData |= (1lu << 29);                  // context code
        }
        if (info->flags & LLKHF_UP) {
            keyData |= (1lu << 31);                  // transition
        }
        ...
        // handle the message
        if (keyboardHookHandler(keyParam, keyData)) {
            return 1;
        }
    }
...
static bool mouseHookHandler(WPARAM wParam, std::int32_t x, std::int32_t y, std::int32_t data)
{
    ...
    case WM_NCXBUTTONUP:
        ...
        return (mode.value() == kHOOK_RELAY_EVENTS);
    ...
    case WM_MOUSEMOVE:
        if (mode.value() == kHOOK_RELAY_EVENTS) {
            ...
            return true;
        } else if (mode.value() == kHOOK_WATCH_JUMP_ZONE) {
...
MSWindowsHook::install()
{
    ...
    // setup immune keys
    g_immuneKeysPath = inputleap::path_to_utf8(inputleap::DataDirectories::profile() / "ImmuneKeys.txt");
