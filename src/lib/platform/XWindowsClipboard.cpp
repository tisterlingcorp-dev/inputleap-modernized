    ...
    m_timeLost(0)
{
    ...
    m_converters.push_back(new XWindowsClipboardPNGFromBitmapConverter(m_display));
    ...
    m_converters.push_back(new XWindowsClipboardFileListConverter(
                ...
                Atom target, ::Time time, Atom property)
{
    ...
    if (target == m_atomTargets) {
        ...
        if (converter != nullptr) {
            ...
            const std::string& clipboardData = m_data[clipboardFormat];
            if (m_added[clipboardFormat] && converter->canConvertFromIClipboard(clipboardData)) {
                try {
                    data = converter->fromIClipboard(clipboardData);
                    if (clipboardData.empty() || !data.empty()) {
                        format = converter->getDataSize();
                        type   = converter->getAtom();
                        LOG_DEBUG("clipboard: serving target %s, bytes=%zu",
...
XWindowsClipboard::getConverter(Atom target, bool onlyIfNotAdded) const
{
    for (auto index = m_converters.begin(); index != m_converters.end(); ++index) {
        ...
        const bool added = m_added[converter->getFormat()];
        if ((!onlyIfNotAdded && added) || (onlyIfNotAdded && !added)) {
            return converter;
        }
    }

    LOG_DEBUG1("  no usable converter for target %s", XWindowsUtil::atomToString(m_display, target).c_str());
    return nullptr;
}
...
XWindowsClipboard::icccmFillCache()
{
    ...
    for (auto index = m_converters.begin(); index != m_converters.end(); ++index) {
        ...
        if (!converter->canConvertToIClipboard()) {
            continue;
        }
...
Atom XWindowsClipboard::getTargetsData(std::string& data, int* format) const
{
    ...
    std::set<Atom> addedTargets{m_atomTargets, m_atomMultiple, m_atomTimestamp};

    // add targets we can convert to
    for (auto index = m_converters.begin(); index != m_converters.end(); ++index) {
        IXWindowsClipboardConverter* converter = *index;

        // skip formats we don't have or cannot convert
        const IClipboard::EFormat clipboardFormat = converter->getFormat();
        const Atom target = converter->getAtom();
        if (m_added[clipboardFormat] &&
            converter->canConvertFromIClipboard(m_data[clipboardFormat]) &&
            addedTargets.insert(target).second) {
            XWindowsUtil::appendAtomData(data, target);
        }
    }
