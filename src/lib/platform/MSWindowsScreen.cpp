...
{
    const auto maximumOrigin = (std::max)(
        virtualOrigin, virtualOrigin + virtualExtent - windowSize);
    return (std::clamp)(
        cursor - windowSize / 2, virtualOrigin, maximumOrigin);
}
...
{
    // Notify the client whenever an external application changes the
    // clipboard.  The previous code only notified after InputLeap had
    // owned the clipboard once, which dropped the first CF_HDROP copied
...
MSWindowsScreen::fakeLocalKey(KeyButton button, bool press) const
{
    ...
    input.ki.wVk = static_cast<WORD>(m_keyState->mapButtonToVirtualKey(button));
...
std::string& MSWindowsScreen::getDraggingFilename()
{
    if (m_draggingStarted) {
        ...
        const std::int32_t xPos = calculateDropWindowOrigin(
            m_isPrimary ? m_xCursor : m_xCenter, m_x, m_w, m_dropWindowSize);
        const std::int32_t yPos = calculateDropWindowOrigin(
            m_isPrimary ? m_yCursor : m_yCenter, m_y, m_h, m_dropWindowSize);
        SetWindowPos(
            m_dropWindow,
            HWND_TOPMOST,
            xPos,
