...
IpcLogOutputter::write(ELevel, const char* text)
{
    ...
    if (m_bufferThread != nullptr &&
        Thread::getCurrentThread().getID() == m_bufferThreadId.load()) {
        return true;
    }
...
{
    m_bufferThreadId.store(Thread::getCurrentThread().getID());

    try {
        while (isRunning()) {
            std::unique_lock<std::mutex> lock(notify_mutex_);
            notify_cv_.wait(lock, [this] {
                return m_bufferWaiting || !m_running.load();
            });
            m_bufferWaiting = false;
            lock.unlock();
            if (!isRunning()) break;
...
IpcLogOutputter::sendBuffer()
{
    ...
    const auto chunk = getChunk(kMaxSendLines);
    if (chunk.empty()) {
        return;
    }
    IpcLogLineMessage message(chunk);
