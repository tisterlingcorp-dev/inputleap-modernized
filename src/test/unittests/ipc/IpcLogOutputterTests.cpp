...
TEST(IpcLogOutputterTests, idleThreadedOutputterClosesWithoutWaitingForTimeout)
{
    MockIpcServer mockServer;
    ON_CALL(mockServer, hasClients(_)).WillByDefault(Return(false));

    const auto started = std::chrono::steady_clock::now();
    {
        IpcLogOutputter outputter(mockServer, kIpcClientGui, true);
    }
