...
{
    if (name == "CBYE") {
        return {name, kMsgCClose};
    }
    if (name == "EBSY") {
        return {name, kMsgEBusy};
    }
    if (name == "EUNK") {
        return {name, kMsgEUnknown};
    }
    if (name == "EBAD") {
        return {name, kMsgEBad};
    }
    throw std::invalid_argument("message must be CBYE, EBSY, EUNK, or EBAD");
}
...
std::vector<std::uint8_t> oracleBytes(const MessageSpec& message)
{
    ...
    if (bytes.size() != 4) {
        throw std::length_error("ProtocolUtil::writef oracle did not emit exactly four bytes");
    }
    return bytes;
}
...
{
    if (argc != 4) {
        std::cerr << "usage: rust-r2-terminal-control-interop <emit|fixture|decode> "
                     "<CBYE|EBSY|EUNK|EBAD> <path>\n";
        return 2;
    }
    ...
    try {
        ...
        if (mode == "emit") {
            ...
            std::cerr << "rust-r2-terminal-control-interop: unknown mode\n";
            return 2;
        }
        return 0;
    }
    catch (const std::exception& error) {
        std::cerr << "rust-r2-terminal-control-interop: " << error.what() << '\n';
        return 1;
    }
}
