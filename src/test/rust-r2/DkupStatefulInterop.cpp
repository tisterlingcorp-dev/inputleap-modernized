...
std::vector<std::uint8_t> dkup(std::uint16_t key, std::uint16_t mask,
                               std::uint16_t button) {
    ScriptedStream stream;
    ProtocolUtil::writef(&stream, kMsgDKeyUp, static_cast<int>(key),
...
void normalCase() {
    ...
    ClientArgs args;
    RecordingClient client(&events, "rust-r2-dkup", NetworkAddress(), &factory, &screen, args);
    client.m_mock = true;
    ScriptedStream stream;
    ServerProxy proxy(&client, &stream, &events);
    stream.append(options());
    stream.append(dkup(0x1234, 0x8001, 0xffff));
    ...
    if (client.keyUps() != expected)
        throw std::runtime_error("DKUP callback field/order mismatch");
    if (client.options() != 1 || client.resets() != 1 || stream.remaining() != 0)
        throw std::runtime_error("DKUP did not continue through trailing CROP");
    if (stream.output() != repeatedNoop(3) || stream.closed() ||
        events.saw(EventType::CLIENT_CONNECTION_FAILED))
        throw std::runtime_error("normal DKUP CNOP/connection behavior mismatch");
}

void malformedLengthCase(std::size_t length) {
    const auto complete = dkup(0x1234, 0x8001, 0xffff);
    if (length < 4 || length >= complete.size())
        throw std::logic_error("malformed DKUP length must be 4..9");
    SpyEventQueue events;
    FakeScreen screen;
    FakeSocketFactory factory;
    ClientArgs args;
    RecordingClient client(&events, "rust-r2-dkup-malformed", NetworkAddress(), &factory, &screen,
                           args);
    client.m_mock = true;
    ScriptedStream stream;
    ServerProxy proxy(&client, &stream, &events);
    ...
    if (!client.keyUps().empty() || stream.output() != emit(kMsgEBad) ||
        !events.saw(EventType::CLIENT_CONNECTION_FAILED) ||
        events.saw(EventType::CLIENT_DISCONNECTED))
        throw std::runtime_error("truncated DKUP was not rejected via connection-failed event");
}
...
int main() {
    try {
        ...
        std::cout << "RUST_R2_DKUP_STATEFUL_PASS dispatch=PUBLIC_STREAM_INPUT_READY "
        ...
        std::cerr << "rust-r2-dkup-stateful: " << error.what() << '\n';
        return 1;
    }
}
