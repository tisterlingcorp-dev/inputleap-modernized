...
std::vector<std::uint8_t> payload(std::int16_t x, std::int16_t y) {
    Capture capture;
    ProtocolUtil::writef(&capture, kMsgDMouseMove, static_cast<int>(x), static_cast<int>(y));
    if (capture.bytes().size() != kPayloadSize ||
        !std::equal(capture.bytes().begin(), capture.bytes().begin() + 4, "DMMV"))
        throw std::logic_error("ProtocolUtil DMMV writer emitted unexpected payload");
...
std::vector<std::uint8_t> frame(std::int16_t x, std::int16_t y) {
    ...
    if (capture->bytes().size() != kFrameSize) throw std::logic_error("PacketStreamFilter DMMV frame size mismatch");
    return capture->bytes();
}
...
std::vector<std::uint8_t> readExact(const std::filesystem::path& path, std::size_t size) {
    ...
    if (!input || input.tellg() != static_cast<std::streamoff>(size)) throw std::runtime_error("input is not an exact DMMV file");
    std::vector<std::uint8_t> bytes(size);
    input.seekg(0);
    input.read(reinterpret_cast<char*>(bytes.data()), static_cast<std::streamsize>(size));
    if (!input) throw std::runtime_error("unable to read exact DMMV input");
    return bytes;
}
}
...
int main(int argc, char** argv) {
    if (argc != 5) {
        std::cerr << "usage: rust-r2-dmmv-interop <emit-frame|fixture-frame|decode-frame> <x> <y> <path>\n";
        return 2;
    }
    try {
        inputleap::Log log;
        ...
        if (mode == "emit-frame") inputleap::writeFile(path, expected, false);
            ...
            if (inputleap::readExact(path, expected.size()) != expected) throw std::runtime_error("bytes differ from C++ DMMV frame oracle");
        }
        else { std::cerr << "rust-r2-dmmv-interop: unknown mode\n"; return 2; }
        std::cout << "DMMV_CPP_" << (mode == "decode-frame" ? "DECODE" : "EMIT") << "_PASS x=" << x << " y=" << y << " frame_bytes=" << expected.size() << "\n";
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "rust-r2-dmmv-interop: " << error.what() << '\n';
        return 1;
    }
}
