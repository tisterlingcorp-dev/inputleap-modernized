...
namespace inputleap {
namespace {
constexpr std::size_t kPayloadSize = 12;
...
std::vector<std::uint8_t> payload(std::uint16_t keyId, std::uint16_t modifierMask,
    ...
    Capture capture;
    ProtocolUtil::writef(&capture, kMsgDKeyRepeat, static_cast<int>(keyId),
                         static_cast<int>(modifierMask), static_cast<int>(count),
                         static_cast<int>(button));
    if (capture.bytes().size() != kPayloadSize ||
        !std::equal(capture.bytes().begin(), capture.bytes().begin() + 4, "DKRP"))
        throw std::logic_error("ProtocolUtil DKRP writer emitted unexpected payload");
    return capture.bytes();
}

std::vector<std::uint8_t> frame(std::uint16_t keyId, std::uint16_t modifierMask,
                                std::uint16_t count, std::uint16_t button) {
    const auto bytes = payload(keyId, modifierMask, count, button);
    Events events;
    ...
    if (capture->bytes().size() != kFrameSize)
        throw std::logic_error("PacketStreamFilter DKRP frame size mismatch");
    return capture->bytes();
}
...
std::vector<std::uint8_t> readExact(const std::filesystem::path& path, std::size_t size) {
    ...
    if (!input || input.tellg() != static_cast<std::streamoff>(size))
        throw std::runtime_error("input is not an exact DKRP file");
    std::vector<std::uint8_t> bytes(size);
    input.seekg(0);
    input.read(reinterpret_cast<char*>(bytes.data()), static_cast<std::streamsize>(size));
    if (!input) throw std::runtime_error("unable to read exact DKRP input");
    return bytes;
}
}
} // namespace inputleap
...
int main(int argc, char** argv) {
    if (argc != 7) {
        std::cerr << "usage: rust-r2-dkrp-interop <emit-frame|fixture-frame|decode-frame> "
                     "<key-id> <modifier-mask> <count> <button> <path>\n";
        return 2;
    }
    try {
        inputleap::Log log;
        ...
        const auto count = inputleap::parseU16(argv[4]);
        const auto button = inputleap::parseU16(argv[5]);
        const std::filesystem::path path(argv[6]);
        const std::string_view mode(argv[1]);
        const auto expected = inputleap::frame(keyId, modifierMask, count, button);
        if (mode == "emit-frame") inputleap::writeFile(path, expected, false);
            ...
            if (inputleap::readExact(path, expected.size()) != expected)
                throw std::runtime_error("bytes differ from C++ DKRP frame oracle");
        }
        else {
            std::cerr << "rust-r2-dkrp-interop: unknown mode\n";
            return 2;
        }
        std::cout << "DKRP_CPP_" << (mode == "decode-frame" ? "DECODE" : "EMIT")
        ...
        std::cerr << "rust-r2-dkrp-interop: " << error.what() << '\n';
        return 1;
    }
}
