...
std::vector<std::uint8_t> payload(std::uint16_t keyId, std::uint16_t modifierMask,
                                  std::uint16_t button) {
    Capture capture;
    ProtocolUtil::writef(&capture, kMsgDKeyUp, static_cast<int>(keyId),
                         static_cast<int>(modifierMask), static_cast<int>(button));
    if (capture.bytes().size() != kPayloadSize ||
        !std::equal(capture.bytes().begin(), capture.bytes().begin() + 4, "DKUP"))
        throw std::logic_error("ProtocolUtil DKUP writer emitted unexpected payload");
...
std::vector<std::uint8_t> frame(std::uint16_t keyId, std::uint16_t modifierMask,
    ...
    if (capture->bytes().size() != kFrameSize)
        throw std::logic_error("PacketStreamFilter DKUP frame size mismatch");
    return capture->bytes();
}
...
std::vector<std::uint8_t> readExact(const std::filesystem::path& path, std::size_t size) {
    ...
    if (!input || input.tellg() != static_cast<std::streamoff>(size))
        throw std::runtime_error("input is not an exact DKUP file");
    std::vector<std::uint8_t> bytes(size);
    input.seekg(0);
    input.read(reinterpret_cast<char*>(bytes.data()), static_cast<std::streamsize>(size));
    if (!input) throw std::runtime_error("unable to read exact DKUP input");
    return bytes;
}
}
} // namespace inputleap
...
int main(int argc, char** argv) {
    if (argc != 6) {
        std::cerr << "usage: rust-r2-dkup-interop <emit-frame|fixture-frame|decode-frame> "
    ...
    try {
        ...
        if (mode == "emit-frame") inputleap::writeFile(path, expected, false);
            ...
            if (inputleap::readExact(path, expected.size()) != expected)
                throw std::runtime_error("bytes differ from C++ DKUP frame oracle");
        }
        else {
            std::cerr << "rust-r2-dkup-interop: unknown mode\n";
            return 2;
        }
        std::cout << "DKUP_CPP_" << (mode == "decode-frame" ? "DECODE" : "EMIT")
                  << "_PASS key_id=" << keyId << " modifier_mask=" << modifierMask
                  << " button=" << button << " frame_bytes=" << expected.size() << "\n";
        return 0;
    }
    catch (const std::exception& error) {
        std::cerr << "rust-r2-dkup-interop: " << error.what() << '\n';
        return 1;
    }
}
