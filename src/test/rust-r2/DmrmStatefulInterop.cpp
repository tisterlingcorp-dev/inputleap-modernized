...
public:
    ...
    void mouseRelativeMove(std::int32_t x, std::int32_t y) override { moves_.emplace_back(x, y); }
...
std::vector<std::uint8_t> dmrm(std::int16_t x, std::int16_t y) {
    ScriptedStream stream;
    ProtocolUtil::writef(&stream, kMsgDMouseRelMove, static_cast<std::uint16_t>(x), static_cast<std::uint16_t>(y));
...
void normalCase() {
    ...
    RecordingClient client(&events, "rust-r2-dmrm", NetworkAddress(), &factory, &screen, args); client.m_mock = true;
    ScriptedStream stream; ServerProxy proxy(&client, &stream, &events);
    stream.append(options());
    stream.append(dmrm((std::numeric_limits<std::int16_t>::min)(), (std::numeric_limits<std::int16_t>::max)()));
    ...
    if (client.moves() != expected) throw std::runtime_error("signed DMRM callback order mismatch");
    if (client.options() != 1 || client.resets() != 1 || stream.remaining() != 0) throw std::runtime_error("DMRM did not continue through trailing CROP");
    if (stream.output() != repeatedNoop(3) || stream.closed() || events.saw(EventType::CLIENT_CONNECTION_FAILED)) throw std::runtime_error("normal DMRM CNOP/disconnect behavior mismatch");
}

void malformedLengthCase(std::size_t length) {
    const auto complete = dmrm((std::numeric_limits<std::int16_t>::min)(),
                               (std::numeric_limits<std::int16_t>::max)());
    if (length < 4 || length >= complete.size())
        throw std::logic_error("malformed DMRM length must be 4..7");
    SpyEventQueue events; FakeScreen screen; FakeSocketFactory factory; ClientArgs args;
    RecordingClient client(&events, "rust-r2-dmrm-malformed", NetworkAddress(), &factory, &screen, args); client.m_mock = true;
    ScriptedStream stream; ServerProxy proxy(&client, &stream, &events);
    stream.append(options());
    ...
    if (!client.moves().empty() || stream.output() != emit(kMsgEBad) ||
        !events.saw(EventType::CLIENT_CONNECTION_FAILED) ||
        events.saw(EventType::CLIENT_DISCONNECTED))
        throw std::runtime_error("truncated DMRM was not rejected fail-closed");
}
...
int main() {
    try { inputleap::Log log; inputleap::rust_r2::normalCase(); inputleap::rust_r2::malformedCases(); std::cout << "RUST_R2_DMRM_STATEFUL_PASS dispatch=PUBLIC_STREAM_INPUT_READY callbacks=MIN_MAX_THEN_NEGATIVE_ZERO trailing_CROP=PROCESSED parse=NORMAL_NONTERMINAL replies=THREE_CNOP malformed_lengths_4_to_7=EBAD_CONNECTION_FAILED_NO_CALLBACK client_disconnected_event=NOT_CLAIMED os_input=NOT_INVOKED\n"; return 0; }
    catch (const std::exception& error) { std::cerr << "rust-r2-dmrm-stateful: " << error.what() << '\n'; return 1; }
}
