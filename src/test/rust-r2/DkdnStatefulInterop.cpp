...
using KeyCall = std::tuple<KeyID, KeyModifierMask, KeyButton>;
...
std::vector<std::uint8_t> dkdn(std::uint16_t key, std::uint16_t mask,
                               std::uint16_t button) {
    ScriptedStream stream;
    ProtocolUtil::writef(&stream, kMsgDKeyDown, static_cast<int>(key),
                         static_cast<int>(mask), static_cast<int>(button));
    return stream.output();
}
...
void normalCase() {
    ...
    ClientArgs args;
    RecordingClient client(&events, "rust-r2-dkdn", NetworkAddress(), &factory, &screen, args);
    client.m_mock = true;
    ScriptedStream stream;
    ServerProxy proxy(&client, &stream, &events);
    stream.append(options());
    stream.append(dkdn(0x1234, 0x8001, 0xffff));
    ...
    const std::vector<KeyCall> expected{
        {static_cast<KeyID>(0x1234), static_cast<KeyModifierMask>(0x8001),
         static_cast<KeyButton>(0xffff)},
        {static_cast<KeyID>(0), static_cast<KeyModifierMask>(0), static_cast<KeyButton>(1)}};
    if (client.keyDowns() != expected)
        throw std::runtime_error("DKDN callback field/order mismatch");
    if (client.options() != 1 || client.resets() != 1 || stream.remaining() != 0)
        throw std::runtime_error("DKDN did not continue through trailing CROP");
    if (stream.output() != repeatedNoop(3) || stream.closed() ||
        events.saw(EventType::CLIENT_CONNECTION_FAILED))
        throw std::runtime_error("normal DKDN CNOP/connection behavior mismatch");
}

void malformedLengthCase(std::size_t length) {
    const auto complete = dkdn(0x1234, 0x8001, 0xffff);
    if (length < 4 || length >= complete.size())
        throw std::logic_error("malformed DKDN length must be 4..9");
    SpyEventQueue events;
    FakeScreen screen;
    FakeSocketFactory factory;
    ClientArgs args;
    RecordingClient client(&events, "rust-r2-dkdn-malformed", NetworkAddress(), &factory, &screen,
                           args);
    client.m_mock = true;
    ScriptedStream stream;
    ServerProxy proxy(&client, &stream, &events);
    ...
    if (!client.keyDowns().empty() || stream.output() != emit(kMsgEBad) ||
        !events.saw(EventType::CLIENT_CONNECTION_FAILED) ||
        events.saw(EventType::CLIENT_DISCONNECTED))
        throw std::runtime_error("truncated DKDN was not rejected via connection-failed event");
}

void malformedCases() {
    for (std::size_t length = 4; length < 10; ++length)
...
int main() {
    try {
        ...
        std::cout << "RUST_R2_DKDN_STATEFUL_PASS dispatch=PUBLIC_STREAM_INPUT_READY "
                     "callbacks=BOUNDARY_THEN_ZERO_ONE trailing_CROP=PROCESSED "
                     "parse=NORMAL_NONTERMINAL replies=THREE_CNOP "
                     "malformed_lengths_4_to_9=EBAD_CONNECTION_FAILED_NO_CALLBACK "
                     "client_disconnected_event=NOT_CLAIMED os_input=NOT_INVOKED\n";
        return 0;
    }
    catch (const std::exception& error) {
        std::cerr << "rust-r2-dkdn-stateful: " << error.what() << '\n';
        return 1;
    }
}
