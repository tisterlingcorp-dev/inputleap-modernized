...
{
    if (name == "DMDN") {
        return {name, kMsgDMouseDown};
    }
    if (name == "DMUP") {
        return {name, kMsgDMouseUp};
    }
    throw std::invalid_argument("message must be DMDN or DMUP");
}
...
std::uint8_t parseRaw(std::string_view value)
{
    ...
    if (value.empty() || result.ec != std::errc{} || result.ptr != value.data() + value.size() ||
        ...
        throw std::invalid_argument("mouse button raw value must be a decimal u8 (0..255)");
    }
    return static_cast<std::uint8_t>(parsed);
}

std::vector<std::uint8_t> oraclePayload(const MessageSpec& message, std::uint8_t raw)
{
    CaptureStream stream;
    ProtocolUtil::writef(&stream, message.symbol, static_cast<unsigned int>(raw));
    if (stream.bytes().size() != kPayloadSize || stream.bytes().back() != raw) {
        throw std::logic_error(std::string(message.name) +
                               " ProtocolUtil writer oracle emitted unexpected bytes");
    }
    return stream.bytes();
}

std::vector<std::uint8_t> oracleFrame(const MessageSpec& message, std::uint8_t raw)
{
    const auto payload = oraclePayload(message, raw);
    ...
    if (frame.size() != kFrameSize) {
        throw std::logic_error(std::string(message.name) +
                               " PacketStreamFilter writer oracle emitted unexpected size");
    }
    return frame;
}
                                         ...
                                         std::size_t expected_size)
{
    ...
    if (end < 0 || static_cast<std::uintmax_t>(end) != expected_size) {
        throw std::length_error("input file size does not match expected mouse-button size");
    }
    ...
    if (input.gcount() != 0) {
        throw std::length_error("input file grew beyond expected mouse-button size");
    }
    if (!input.eof()) {
...
void emit(const MessageSpec& message, std::uint8_t raw, const std::filesystem::path& path,
          bool fixture_mode, bool framed)
{
    const auto bytes = framed ? oracleFrame(message, raw) : oraclePayload(message, raw);
    writeBytes(path, bytes, fixture_mode);
    std::cout << message.name << (framed ? "_CPP_FRAME_EMIT_PASS" : "_CPP_EMIT_PASS")
              << " raw=" << static_cast<unsigned int>(raw) << " bytes=" << bytes.size()
              << " path=" << path << '\n';
}

void decode(const MessageSpec& message, std::uint8_t raw, const std::filesystem::path& path,
            bool framed)
{
    const auto expected = framed ? oracleFrame(message, raw) : oraclePayload(message, raw);
    const auto bytes = readExactBytes(path, expected.size());
    if (!std::equal(bytes.begin(), bytes.end(), expected.begin(), expected.end())) {
        throw std::runtime_error("bytes do not match the real mouse-button writer oracle");
    }
    std::cout << message.name << (framed ? "_CPP_FRAME_DECODE_PASS" : "_CPP_DECODE_PASS")
...
{
    if (argc != 5) {
        std::cerr << "usage: rust-r2-mouse-button-interop "
                     "<emit|fixture|decode|emit-frame|decode-frame> "
                     "<DMDN|DMUP> <0..255> <path>\n";
        return 2;
    }
    ...
    try {
        ...
        const auto raw = inputleap::rust_r2::parseRaw(argv[3]);
        if (mode == "emit") {
            inputleap::rust_r2::emit(message, raw, argv[4], false, false);
        }
        else if (mode == "fixture") {
            inputleap::rust_r2::emit(message, raw, argv[4], true, false);
        }
        else if (mode == "decode") {
            inputleap::rust_r2::decode(message, raw, argv[4], false);
        }
        else if (mode == "emit-frame") {
            inputleap::rust_r2::emit(message, raw, argv[4], false, true);
        }
        else if (mode == "decode-frame") {
            inputleap::rust_r2::decode(message, raw, argv[4], true);
        }
        else {
            std::cerr << "rust-r2-mouse-button-interop: unknown mode\n";
            return 2;
        }
        return 0;
    }
    catch (const std::exception& error) {
        std::cerr << "rust-r2-mouse-button-interop: " << error.what() << '\n';
        return 1;
    }
}
