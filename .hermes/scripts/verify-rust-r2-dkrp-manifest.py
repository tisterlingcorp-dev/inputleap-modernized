#!/usr/bin/env python3
"""Fail-closed verifier for the frozen R2.12 DKRP differential contract."""
...
MANIFEST = FIXTURES / "manifest-r2.12-dkrp.json"
EXPECTED_FRAME = bytes.fromhex("0000000c444b525012348001ffff0002")
EXPECTED_CONTEXTS = {
...
def verify() -> None:
    ...
    require(data["schema"] == 1 and data["tranche"] == "R2.12", "schema/tranche mismatch")
    require(data["rustStatefulCoverage"] == "none", "Rust stateful overclaim")
    require("RecordingClient keyRepeat override" in data["cppStatefulCoverage"], "missing safe C++ double")
    require(
        "every partial DKRP payload length 4 through 11" in data["cppStatefulCoverage"],
    ...
    require(len(fixtures) == 1, "exactly one DKRP fixture required")
    fixture = fixtures[0]
    require(
        (
            fixture["keyId"],
            fixture["modifierMask"],
            fixture["count"],
            fixture["button"],
            fixture["size"],
        )
        == (0x1234, 0x8001, 0xFFFF, 2, 16),
    ...
    require(contract["messages"] == ["DKRP"], "message scope mismatch")
    require(contract["formats"] == ["DKRP%2i%2i%2i%2i"], "format mismatch")
    require(contract["cppSymbols"] == ["kMsgDKeyRepeat"], "C++ symbol mismatch")
    require(
        contract["fieldTypes"]
        == ["keyId:u16", "modifierMask:u16", "count:u16", "button:u16"],
        "field type mismatch",
    )
    ...
    require(contract["boundary"] == 12 and contract["terminal"] is False, "boundary/terminal mismatch")
    ...
    require(
        any(
            "every partial DKRP payload length 4 through 11" in case
    ...
    require(
        any("DMRM and DMWM remain outside R2.12" in item for item in data["scopeExclusions"]),
    ...
    stateful_harness = (REPO / "src/test/rust-r2/DkrpStatefulInterop.cpp").read_text(encoding="utf-8")
    attributes = (REPO / ".gitattributes").read_text(encoding="utf-8")
    require('b"DKRP"' in rust_lib and "RemoteMessage::KeyRepeat" in rust_lib, "Rust DKRP codec absent")
    require("dkrp_context_matrix_is_only_server_to_client_active" in rust_tests, "context test absent")
    require("every_short_dkrp_payload_is_malformed_without_boundary" in rust_tests, "truncation test absent")
    require(
        'throw XBadClient("truncated key repeat message")' in server_proxy,
        "C++ truncation hardening absent",
    )
    require(
        "for (std::size_t length = 4; length < 12; ++length)" in stateful_harness,
    ...
    print(
        "R2_12_DKRP_MANIFEST_PASS files=4 r0_sha256="
        f"{data['sourceManifestSha256']} payload=12 frame=16 contexts=1of8 "
        "truncated_lengths=4_to_11_EBAD_CONNECTION_FAILED_NO_CALLBACK "
...
def main() -> int:
    try:
        ...
        print(f"R2_12_DKRP_MANIFEST_FAIL: {error}", file=sys.stderr)
        return 1
