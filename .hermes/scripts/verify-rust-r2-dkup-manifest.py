#!/usr/bin/env python3
"""Fail-closed verifier for the frozen R2.11 DKUP differential contract."""
...
MANIFEST = FIXTURES / "manifest-r2.11-dkup.json"
EXPECTED_FRAME = bytes.fromhex("0000000a444b555012348001ffff")
EXPECTED_CONTEXTS = {
...
def verify() -> None:
    ...
    require(data["schema"] == 1 and data["tranche"] == "R2.11", "schema/tranche mismatch")
    require(data["rustStatefulCoverage"] == "none", "Rust stateful overclaim")
    require("RecordingClient keyUp override" in data["cppStatefulCoverage"], "missing safe C++ double")
    require(
        "every partial DKUP payload length 4 through 9" in data["cppStatefulCoverage"],
    ...
    require(len(fixtures) == 1, "exactly one DKUP fixture required")
    fixture = fixtures[0]
    require(
    ...
    require(contract["messages"] == ["DKUP"], "message scope mismatch")
    require(contract["formats"] == ["DKUP%2i%2i%2i"], "format mismatch")
    require(contract["cppSymbols"] == ["kMsgDKeyUp"], "C++ symbol mismatch")
    require(
    ...
    require(
        any(
            "every partial DKUP payload length 4 through 9" in case
    ...
    require(any("DKRP remains outside R2.11" in item for item in data["scopeExclusions"]), "sibling scope exclusion missing")
    ...
    stateful_harness = (REPO / "src/test/rust-r2/DkupStatefulInterop.cpp").read_text(encoding="utf-8")
    attributes = (REPO / ".gitattributes").read_text(encoding="utf-8")
    require('b"DKUP"' in rust_lib and "RemoteMessage::KeyUp" in rust_lib, "Rust DKUP codec absent")
    require("dkup_context_matrix_is_only_server_to_client_active" in rust_tests, "context test absent")
    require("every_short_dkup_payload_is_malformed_without_boundary" in rust_tests, "truncation test absent")
    require('throw XBadClient("truncated key up message")' in server_proxy, "C++ truncation hardening absent")
    require(
    ...
    print(
        "R2_11_DKUP_MANIFEST_PASS files=4 r0_sha256="
...
def main() -> int:
    try:
        ...
        print(f"R2_11_DKUP_MANIFEST_FAIL: {error}", file=sys.stderr)
        return 1
